...
def download_data_with_retry(symbols, start_date=None, end_date=None, max_retries=5, delay=2.0):
    """
            ...
            if df.empty:
                if attempt < max_retries - 1:
                    ...
                    if len(symbols) > 1 and attempt == max_retries - 1:
                        ...
                        return download_individual_symbols(symbols, start_date, end_date, max_retries//2, delay)
                    return pd.DataFrame()
...
def download_individual_symbols(symbols, start_date=None, end_date=None, max_retries=3, delay=1.0):
    """Helper function to download symbols individually"""
    result_dfs = []
...
def calculate_momentum_scores(start_date=None, end_date=None, use_cache=True, custom_file=None):
    ...
    momentum_values = []
    for symbol in prices.index.get_level_values("symbol").unique():
        if VERBOSE:
            print(f"\nProcessing momentum for {symbol}")
